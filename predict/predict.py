...
model_path = Path(__file__).parent / "ukm_30min_lstm.pkl"
data_path = Path(__file__).parent / "ukm_30min_prepared.csv"
...
def predict(df: pd.DataFrame) -> list[pd.DataFrame]:
    ts_data = TimeSeriesData(
        ...
        history_horizon=model.history_horizon,  # HINT: All the data is input data for one predicition.
        ...
        preparation_steps=[  # HINT: Order is important
            partial(dh.set_to_hours, freq="30min", timecolumn="Time"),  # HINT: adjust freq to forecast frequency
            partial(
                dh.fill_if_missing, periodicity=config.get("periodicity", 48)
            ...
            partial(dh.add_onehot_features, timestep="30min"),
